...
export const obfuscateHTML = (sourceString, _log, logMessage) => {
    try {
        ...
        for (var i=0; i<result.length; i++) {
            ...
            if(maxLength > 10000) {
                let percent = (i / maxLength) * 100
                let terminalOut = `\x1b[2m\x1b[43m\x1b[30mProgress: (${i}/${maxLength} | ${Math.floor(percent)}%)\x1b[0m ${logMessage}`;
                _log(terminalOut.length > process.stdout.columns ?
        ...
        let obfuscate = `<script>${obfuscateJS(jsPreset).source}</script><noscript>You must enable JavaScript to see this text.</noscript>`
