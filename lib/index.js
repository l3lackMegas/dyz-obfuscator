...
export const obfuscateHTML = (sourceString) => {
    try {
        ...
        for (var i=0; i<result.length; i++) {
            arrString.push(`x${result.charCodeAt(i) + 38960}`)
            encode += String.fromCharCode(result.charCodeAt(i) + 1);
        }
        encode = encode.replace(/(`)/g, '\\`')
        let obfuscate = `<script>var s=\`${JSON.stringify(arrString)}\`;var m="";var mm=JSON.parse(s);for(var i=0;i<mm.length;i++)m+=String.fromCharCode(parseInt(mm[i].substring(1,mm[i].length))-38960);document.write(m);setTimeout(()=>{var sc=document.getElementsByTagName('script');while(sc.length > 0)sc[0].remove();},1000);</script><noscript>You must enable JavaScript to see this text.</noscript>`
